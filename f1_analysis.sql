--Number of wins
--notes: change #wins from string to integer 
SELECT DISTINCT d.forename, d.surname,d.driverId, SUM(CAST(r.points AS decimal)) AS Total_points, c.name-- r.laps, r.grid, r.position
FROM drivers AS d
...
WHERE rac.year > 2010
GROUP BY d.forename, d.surname, c.name, d.driverId
...
--Sebastian Vettel Podium by team
...
--USING CASE TO DETERMINE IF drive scored a victory, podium, or poimts.
...
INNER JOIN drivers AS d
ON r.driverId = d.driverId
WHERE d.surname = 'Alonso' AND rac.year = 2022 --AND r.position <> '\N'
GROUP BY  d.forename, d.surname
ORDER BY COUNT(CASE WHEN r.position = '\N' THEN 'DNF'END)*100/COUNT(rac.name)


--NUMBER OF DNFs BY RACE IN AS SEASON
SELECT rac.name, CAST(rac.date AS DATE),
